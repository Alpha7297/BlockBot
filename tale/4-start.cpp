...
const QString GuardSpeakerName = QString::fromUtf8("Patrolman");
...
std::vector<TaleParagraph> buildLevelOneStory(){
    ...
    story.push_back({
        GuardSpeakerName,
        QString::fromUtf8("封锁区域。扫描模式启动。"),
        {{TaleCharacter::Quiver,QPointF(460,180)}}
    });

    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("巡逻者移动速度为零。视野方向持续旋转。视野范围四格。"),
        {{TaleCharacter::Puzzled,QPointF(460,180)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("它们站着不动，但视线一直在动。别只看它们的位置。"),
    ...
    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("需要判断每个巡逻者当前朝向，以及目标格是否在四格视野内。"),
    ...
    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("一个条件不够，就把所有危险条件都列出来。"),
        {{TaleCharacter::Questioning,QPointF(220,180)},
        {TaleCharacter::Echo,QPointF(700,130)}}
    });
    ...
    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("使用多条件分支和布尔判断，避开四个巡逻者的旋转视野，到达右侧走廊。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("先把危险拆成几个条件：是否和巡逻者在同一行或同一列，距离是否不超过四格，巡逻者当前朝向是否正对 R-07。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("用多个 if 或布尔组合判断目标格是否安全。每次移动前先判断，危险就 wait，安全再前进。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });
    return story;
}
