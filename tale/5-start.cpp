...
std::vector<TaleParagraph> buildLevelOneStory(){
    ...
    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("检测到地下迷宫。起点确认。出口信号位于远端。"),
        {{TaleCharacter::Curious,QPointF(460,180)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("正门被关上之后，工厂就只剩这些绕路。别急，迷宫也只是很多个选择排在一起。"),
        {{TaleCharacter::Curious,QPointF(220,180)},
        {TaleCharacter::Echo,QPointF(700,130)}}
    });

    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("需要识别墙体，选择可通行方向，持续向出口推进。"),
    ...
    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("如果一条路走不通，就换下一条。不要把撞墙当成失败，把它当成地图告诉你的信息。"),
        {{TaleCharacter::Puzzled,QPointF(220,180)},
        {TaleCharacter::Echo,QPointF(700,130)}}
    });
    ...
    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("使用前方地图判断、循环和分支，在迷宫中避开墙体，从 (1,1) 到达 (38,38) 的出口。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("使用“前方地图”判断前方是否是墙。如果前方不是墙，就前进；如果前方是墙，就转向再试。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("可以先写一个简单的贴墙走策略：优先尝试右转后的方向，能走就走，不能走再尝试直行、左转或掉头。迷宫每次会生成不同形状，代码不要依赖固定路线。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("迷宫不会解释自己。你的程序要替你记住试过什么。"),
