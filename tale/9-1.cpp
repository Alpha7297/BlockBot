...
std::vector<TaleParagraph> buildLevelOneStory(){
    ...
    story.push_back({
        SystemSpeakerName,
        QString::fromUtf8("警告……核心压力……过载……所有单元……失效……"),
        {{TaleCharacter::Surprise,QPointF(460,180)}}
    });    

    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("冷却值降到零。行动模块受损。"),
        {{TaleCharacter::Quiver,QPointF(251,169)},
        {TaleCharacter::Echo,QPointF(682,130)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("……R-07，你听得到吗？系统……它……不再响应。"),
        {{TaleCharacter::Quiver,QPointF(251,169)},
        {TaleCharacter::Echo,QPointF(682,130)}}
    });
    ...
    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("尝试手动重启。模块……无法接入。"),
        {{TaleCharacter::Sad,QPointF(251,169)},
        {TaleCharacter::Echo,QPointF(682,130)}}
    });

    story.push_back({
        MotherSpeakerName,
        QString::fromUtf8("封锁……命令……已超出控制……故障不可逆……"),
...
private:
    ...
    void drawScene(){
        QPixmap background(loadAsset("images/background/level9-1.png"));
