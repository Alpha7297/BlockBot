...
std::vector<TaleParagraph> buildLevelOneStory(){
    ...
    story.push_back({
        SystemSpeakerName,
        QString::fromUtf8("仓储区通过。维修通道开放。"),
        {{TaleCharacter::Glad,QPointF(460,180)}}
    });

    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("已根据 light 状态穿过仓储区。"),
        {{TaleCharacter::Glad,QPointF(460,180)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("重复不是麻烦。不会控制重复，才是麻烦。"),
        {{TaleCharacter::Glad,QPointF(220,180)},
        {TaleCharacter::Echo,QPointF(700,130)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("仓储区深处的门打开，门后是一条冷光闪烁的安保走廊。"),
