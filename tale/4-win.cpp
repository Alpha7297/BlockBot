...
std::vector<TaleParagraph> buildLevelOneStory(){
    ...
    story.push_back({
        GuardSpeakerName,
        QString::fromUtf8("未检测到入侵目标。继续扫描。"),
        {{TaleCharacter::Determined,QPointF(460,180)}}
    });

    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("已通过安保走廊。"),
        {{TaleCharacter::Glad,QPointF(460,180)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("广播忽然切换成清晰、冰冷的声音。"),
        {{TaleCharacter::Surprise,QPointF(460,180)}}
    });

    story.push_back({
        MotherSpeakerName,
        QString::fromUtf8("维修单元 R-07，返回仓位。"),
        {{TaleCharacter::Surprise,QPointF(220,180)},
        {TaleCharacter::Mother,QPointF(700,105)}}
    });

    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("请求开启最近出口。"),
    ...
    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("最近的出口门落下，挡住前路。"),
        {{TaleCharacter::Quiver,QPointF(460,180)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("它不会让你走正门。去地下管网。"),
        {{TaleCharacter::Determined,QPointF(220,180)},
