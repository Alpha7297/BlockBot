...
std::vector<TaleParagraph> buildLevelOneStory(){
    ...
    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("检测到九块仓储货格。状态列表：light。"),
        {{TaleCharacter::Puzzled,QPointF(460,180)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("这些灯不是装饰。红灯亮着的时候，货格里的地刺还在等你犯错。"),
    ...
    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("红灯：禁止通过。绿灯：地刺收回，可以移动。"),
        {{TaleCharacter::Questioning,QPointF(220,180)},
        {TaleCharacter::Echo,QPointF(700,130)}}
    });

    story.push_back({
        EchoSpeakerName,
        QString::fromUtf8("所以别只看终点。先读 light，确认当前货格安全，再让履带继续往前。"),
        {{TaleCharacter::Questioning,QPointF(220,180)},
        {TaleCharacter::Echo,QPointF(700,130)}}
    });
    ...
    story.push_back({
        RobotSpeakerName,
        QString::fromUtf8("如果当前货格仍为红灯，需要等待扫描完成。绿灯出现后，再进入下一段路径。"),
        {{TaleCharacter::Determined,QPointF(220,180)},
        {TaleCharacter::Echo,QPointF(700,130)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("读取 light 列表，使用 if、wait 和循环，让 R-07 等待货格变绿后再穿过地刺区域。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("本关需要使用列表。light 列表保存九块货格的状态，进入某个货格前，先读取对应灯号。若仍为红灯，就执行 wait 并继续检查；若已经变绿，再移动到下一段路径。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });

    story.push_back({
        EmptySpeakerName,
        QString::fromUtf8("可以用变量记录当前货格编号，让循环按货格顺序推进。"),
        {{TaleCharacter::Robot,QPointF(460,180)}}
    });
    return story;
}
